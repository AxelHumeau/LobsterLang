...
evalAst (Call "+" [AST.Symbol s1, AST.Symbol s2]) =
        Just (AST.Symbol (s1 ++ s2))
evalAst (Call "-" [AST.Integer a, AST.Integer b]) = Just (AST.Integer (a - b))
evalAst (Call "*" [AST.Integer a, AST.Integer b]) = Just (AST.Integer (a * b))
evalAst (Call "/" [AST.Integer a, AST.Integer b]) =
        Just (AST.Integer (div a b))
evalAst (Call "%" [AST.Integer a, AST.Integer b]) =
        Just (AST.Integer (mod a b))
...
evalAst (Call c [t1, t2]) = maybe Nothing evalAst ((Call c) <$>
        (sequence [evalAst t1, evalAst t2]))
evalAst _ = Nothing
